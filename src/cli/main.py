...
from src.cli.display import display_error, display_header
...
def create_parser() -> argparse.ArgumentParser:
    ...
    calculate_parser = subparsers.add_parser(
        "calculate", aliases=["calc"], help="相続計算を実行"
    )
    calculate_parser.add_argument(
        "-i", "--input", dest="input_file", type=Path, help="入力JSONファイルのパス"
    )
    calculate_parser.add_argument(
        "-o", "--output", dest="output", type=Path, help="出力ファイルのパス"
    )
    calculate_parser.add_argument(
        "--save-to-neo4j", action="store_true", help="結果をNeo4jに保存"
    )
    calculate_parser.set_defaults(func=commands.calculate_command)

    # validate コマンド
    validate_parser = subparsers.add_parser("validate", help="入力ファイルを検証")
    validate_parser.add_argument(
        "input_file", type=Path, help="検証する入力JSONファイルのパス"
    )
    validate_parser.set_defaults(func=commands.validate_command)

    # demo コマンド
    demo_parser = subparsers.add_parser("demo", help="デモを実行")
    demo_parser.add_argument(
        ...
        nargs="?",
        help=(
            "デモのタイプ (basic: 基本ケース, complex: 複雑ケース, "
            "interactive: 対話型)"
        ),
    )
    ...
    template_parser = subparsers.add_parser(
        "template", help="テンプレートファイルを作成"
    )
    template_parser.add_argument(
        "output_file", type=Path, help="出力するテンプレートファイルのパス（.csv）"
    )
    template_parser.set_defaults(func=commands.template_command)

    # tree コマンド
    tree_parser = subparsers.add_parser("tree", help="家系図を生成")
    ...
    interview_parser = subparsers.add_parser(
        "interview", help="AI対話型インタビューで相続情報を収集"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        display_header(
            "相続計算機 (Inheritance Calculator)", "日本の民法に基づく相続計算ツール"
        )
        parser.print_help()
        return 0
